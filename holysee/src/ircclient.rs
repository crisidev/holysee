pub mod client {
    ...
    fn main_to_irc_loop(from_main_queue: &Receiver<Message>, server: &IrcServer, channel_name: &String) {
        loop {
    ...
    fn irc_to_main_loop(to_main_queue: &Sender<Message>, server: &IrcServer, channel_name: &String) {
    ...
    pub fn new(settings: &Settings, to_main_queue: Sender<Message>) -> Sender<Message> {
        ...
        thread::spawn( move || irc_to_main_loop(&to_main_queue, &irc_to_main_server, &irc_channel_name));
        thread::spawn( move || main_to_irc_loop(&from_main_queue, &main_to_irc_server, &irc_channel_name_clone));
